...
export default function Modal({ isFormData, isModal }: { isFormData: TFormSchema, isModal: boolean }) {
  return (
    <div
      ...
      w-full overflow-y-scroll ${isModal && "opacity-100 bg-opacity-80 animate-open-menu pointer-events-auto "}`}
    >
      <div className="w-4/5 max-w-xl flex flex-col justify-center gap-8">
        <div className="h-6 relative lg:h-10">
          <div className={`bg-brand-purple h-full absolute ${isModal && "opacity-100 bg-opacity-80 animate-progress"}`} />
        </div>
        <h4>
